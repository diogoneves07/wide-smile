...
import CreatorFnProperties from './creator-fn-properties';
declare type PerformerFn = CreatorFnProperties['performers'][number];
export declare type PropertiesForTheCreationOfAnimation = Partial<Omit<AnimationInstance, 'targets'>> & Pick<AnimationInstanceProperties, 'targets'>;
export interface AnimationInstance extends UserAnimationObjectInternal, AnimationsPropotype {
}
export declare type AnimationInstancePropertiesAllWritable = {
    -readonly [K in keyof AnimationInstanceProperties]: AnimationInstanceProperties[K];
};
export declare type UserAnimationObjectInternal = Required<Omit<AnimationInstancePropertiesAllWritable, 'targets'>> & {
...
export default interface AnimationInstanceProperties extends UserAnimationOptions {
     ...
     * @readonly
     */
    readonly creator?: Function & CreatorFnProperties;
    /**
     * Gets the state in which the animation is.
     *
     * @readonly
     */
    readonly state?: 'waiting' | 'loading' | 'loaded' | 'running' | 'paused' | 'completed' | 'destroyed' | 'canceled';
     ...
     *
     * The priority for style insertion and determined by the animationId property value of the animation instance, how much the higher its value the more important it is style generated by the animation.
     * @readonly
     */
...
export declare type AnimationOptions = Omit<AnimationInstanceProperties, 'loop' | 'dur' | 'delay' | 'endDelay' | 'targets'> & {
    targets?: TargetsAnimationProperty;
    loop?: AnimationInstanceProperties['loop'] | FunctionForPropertyValueInKeyframe | string;
    dur?: AnimationInstanceProperties['dur'] | FunctionForPropertyValueInKeyframe | string;
    delay?: AnimationInstanceProperties['delay'] | FunctionForPropertyValueInKeyframe | string;
